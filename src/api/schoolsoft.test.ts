...
test("bitmaskToWeeks decodes low bits", () => {
...
test("bitmaskToWeeks reaches weeks beyond bit 31", () => {
  assert.deepEqual(bitmaskToWeeks(2 ** 32), [33]);
  assert.deepEqual(bitmaskToWeeks(2 ** 52), [53]);
});

test("bitmaskToWeeks does not alias high weeks onto low ones", () => {
...
test("bitmaskToWeeks covers a full-year mask", () => {
...
test("isTokenExpired treats an unreadable date as expired", () => {
...
test("isTokenExpired reads SchoolSoft's timestamp format", () => {
  assert.equal(isTokenExpired("2000-01-01 00:00:00.0"), true);
  assert.equal(isTokenExpired("2099-01-01 00:00:00.0"), false);
});

test("isTokenExpired applies the five-minute safety margin", () => {
